...
class ArticleAdd extends Component {
  ...
  render() {
    const { categorys, tags, categorysLoading, updateItemLoading, tagsLoading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        <Spin spinning={updateLoading}>
          <Card title={updateItem ? '修改文章' : '撰写文章'}>
            <Form layout="vertical" ref={this.formRef} onFinish={this.handleSubmit}>
                ...
                <Col span={6} push={18}>
                  ...
                  >
                    ...
                    >
                      {categorys.length > 0
                        ? categorys.map(v => (
                            <Option key={`categ-${v.id}`} value={v.id}>
                              {v.name}
                            </Option>
                          ))
                        : ''}
                      ...
                      dropdownRender={menu => (
                        <div>
                          ...
                          <div style={{ display: 'flex', flexWrap: 'nowrap', padding: 8 }}>
                            <Input
                              style={{ flex: 'auto' }}
                              value={this.state.newTagNameIs}
                              onChange={v => this.setState({ newTagNameIs: v.target.value })}
                            />
                            <a
                              style={{
                                flex: 'none',
              ...
              <Form.Item>
                ...
                <Button
                  style={{ marginLeft: 10 }}
                  onClick={() => this.setState({ showCoverModal: true })}
                >
                  添加文章头图
                </Button>
              </Form.Item>
        ...
        >
          <Form ref={this.formRef2} layout="vertical" onFinish={this.onFinish}>
            ...
            <Form.Item
              name="cover"
              label="文章缩略图"
              rules={[{ type: 'url', message: '输入合法的URL' }]}
            >
              <TextArea rows={4} placeholder="输入URL" />
            </Form.Item>
            <Form.Item>
              <Button htmlType="submit" onClick={() => this.setState({ showCoverModal: false })}>
                提交
              </Button>
            </Form.Item>
          </Form>
...
export default connect(({ category: { categorys }, tag: { tags }, loading }) => ({
  categorys,
  tags,
